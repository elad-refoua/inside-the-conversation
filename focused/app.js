...
mobileOptions.onclick=()=>{
 ...
 open('<h2>המסע שלכם</h2><p><a href="'+compare.href+'" target="_blank" rel="noopener">פתיחת אותה תחנה בגרסה הקודמת ↗</a></p><div class="mobile-options-grid">'+options.map(([id,title,detail])=>'<button data-mobile-command="'+id+'"><strong>'+title+'</strong><span>'+detail+'</span></button>').join('')+'</div>');
...
$('#tour').onclick=()=>{if(tour){stopTour();return;}$('#tour').setAttribute('aria-pressed','true');$('#tour').textContent='Ⅱ עצירה';if(index===chapters.length-1)go(0);tour=setInterval(()=>{if(dialog.open||document.hidden)return;if(index===chapters.length-1&&focusStep===chapters[index].focus.steps.length-1){stopTour();return;}advance(1);},12000);};
...
function enlarge(labelIndex=null){
 const c=chapters[index],full=window.FULL_CONVERSATION.chapters[index];stopTour();
 const selected=labelIndex===null?c.labels:[c.labels[labelIndex]];
 open('<p class="zoom-context">גרסה ממוקדת · שקף '+c.slide+' · חלק '+(focusStep+1)+' מתוך '+c.focus.steps.length+'</p><h2>'+escape(c.title)+'</h2>'+selected.map(l=>'<section class="enlarged-panel focus-dialog-step">'+l.html+'</section>').join('')+'<p class="zoom-caption">'+c.caption+'</p><details class="focus-full-details"><summary>הגרפים המלאים, המדידה וההסבר ↗</summary><p class="focus-context">התוכן המלא של התחנה, כפי שנשמר בגרסה הקודמת</p>'+full.labels.map(l=>'<section class="enlarged-panel">'+l.html+'</section>').join('')+additional(full)+'<div class="audited-notes">'+escape(full.notes||'').split(/\n\n+/).map(p=>'<p>'+p+'</p>').join('')+'</div></details>');
 dialog.classList.add('data-dialog');setRevealDOM(dialog,focusStep);
}

zoom.textContent='להרחבה';zoom.title='לחצו על כל גרף או על כותרת השקף להגדלה (G)';zoom.onclick=()=>enlarge();
...
$('#reading').onclick=()=>{open('<h2>בתוך השיחה — גרסה לקריאה</h2>'+chapters.map(c=>`<section class="reading-chapter"><h3>${c.title}</h3>${c.labels.map(l=>`<div class="world-label">${l.html}</div>`).join('')}<p>${c.caption||''}</p>${additional(c)}<p>${c.notes||''}</p></section>`).join(''));dialog.querySelectorAll('[data-reveal]').forEach(el=>el.hidden=false);};
$('#explore').onclick=()=>{explore=!explore;world?.setExplore(explore);$('#explore').setAttribute('aria-pressed',String(explore));$('#explore-hint').hidden=!explore;if(!explore)go(index,false,focusStep);};
...
motionButton();
$('#motion').onclick=()=>{still=!still;world?.setMotion(still);motionButton();if(still)go(index,true,focusStep);};
...
addEventListener('keydown',e=>{if(dialog.open||e.defaultPrevented||e.target.closest('input,select,textarea')||(e.target.closest('#mobile-notes')&&['ArrowUp','ArrowDown','PageUp','PageDown','Home','End',' '].includes(e.key))||(e.key===' '&&e.target.closest('button,a,[role=button]')))return;if(['ArrowLeft','ArrowDown','PageDown',' '].includes(e.key)){e.preventDefault();stopTour();advance(1);}if(['ArrowRight','ArrowUp','PageUp'].includes(e.key)){e.preventDefault();stopTour();advance(-1);}if(e.key==='Home'){stopTour();go(0);}if(e.key==='End'){stopTour();go(chapters.length-1);}if(e.key.toLowerCase()==='m')$('#overview').click();if(e.key.toLowerCase()==='n')$('#sources').click();if(e.key.toLowerCase()==='f')$('#fullscreen').click();});
addEventListener('hashchange',()=>{const parts=location.hash.slice(1).split('.');go((parseInt(parts[0])||1)-1,false,(parseInt(parts[1])||1)-1);});
...
function syncFocus(animate=false){
 ...
 $('#next').disabled=index===chapters.length-1&&last;$('#previous').disabled=index===0&&focusStep===0;
 const previousBase=location.hostname==='127.0.0.1'||location.hostname==='localhost'?'http://127.0.0.1:4180/':'../';compare.href=previousBase+'#'+c.slide;
 $('#mobile-sheet-action').textContent=mobileExpanded?'חזרה לחדר':(c.focus.steps.length>1?'חלק '+(focusStep+1)+' / '+c.focus.steps.length:'פתיחה');
 if(c.slide===12){$('#chapter-purpose').textContent=s.heading+' · '+s.body;measureMobileScene();}
 history.replaceState(null,'','#'+c.slide+(focusStep?'.'+(focusStep+1):''));
}
...
const initialHash=location.hash.slice(1).split('.'),requested=(parseInt(initialHash[0])||1)-1;go(requested,true,(parseInt(initialHash[1])||1)-1);
try{await document.fonts.load('500 30px Assistant');world=await createWorld($('#world'),chapters);world.setMotion(still);go(index,true,focusStep);$('#loading').classList.add('done');$('#loading').setAttribute('aria-hidden','true');window.CONVERSATION_READY=true;window.getConversationState=()=>({...world.getState(),focusStep,focusSteps:chapters[index].focus.steps.length,variant:"focused"});}catch(error){console.error(error);$('#loading-status').textContent='המרחב לא נטען. אפשר לפתוח את גרסת הקריאה מהכפתור למטה.';$('#loading').style.pointerEvents='none';$('#loading').style.zIndex='3';$('#reading').style.display='block';}
